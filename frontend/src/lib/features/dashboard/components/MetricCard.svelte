...
{#if onclick}
  <button
    type="button"
    {onclick}
    class="flex w-full flex-col gap-xs rounded-2xl bg-surface-container-low p-md text-left text-on-surface transition-colors hover:bg-surface-container {klass}"
  >
...
{:else}
  <div
    class="flex w-full flex-col gap-xs rounded-2xl bg-surface-container-low p-md text-left text-on-surface transition-colors {klass}"
  >
    <div class="flex items-center gap-sm">
      {#if icon}<Icon name={icon} size={20} class="text-primary" />{/if}
      <span class="text-label-md font-label-md text-on-surface-variant">{label}</span>
